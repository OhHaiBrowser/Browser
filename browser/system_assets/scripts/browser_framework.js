...
var OhHaiBrowser = {
	...
	tabs: {
		...
		add: function (_URL, _ID, _OPTIONS, callback) {
			let IsNew = false;
			if (_ID == undefined) {
				_ID = OhHaiBrowser.core.generateId();
				IsNew = true;
			}
		...
		groups: {
			add: function (_id, _title, _tab, callback) {
				var this_group;
				if (_id == null) {
					_id = 'group-' + OhHaiBrowser.core.generateId();
				}
			...
			removeTab: function (_tab, callback) {
				...
				var TabSessionId = _tab.getAttribute('data-session');
				Sessions.UpdateParent(TabSessionId, OhHaiBrowser.ui.tabbar.tabcontainer.id, function (id) {
					console.log('RemoveFromGroup - Parent updated:' + thisGroup.id);
				});
